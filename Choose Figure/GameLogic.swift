...
extension GameLogic: GameActions {
    ...
    func userChoose(index: Int) {
        ...
        if figureIndex == rightFigureIndex {
            if let indexInArray = figuresToChoose.firstIndex(of: index) {
                figuresToChoose.remove(at: indexInArray)
                delegate?.userDidRightChoice(index: index) { [weak self] in
                    guard let self = self else { return }
                    if self.figuresToChoose.count == 0 {
                        self.delegate?.moveToNextLevel()
                    }
                }
            }
        } else {
            self.delegate?.userDidWrongChoice(index: index) { }
            self.delegate?.lives -= 1
            if self.delegate?.lives == 0 {
                self.delegate?.gameOver()
            }
        }
        score = delegate?.level ?? 0
    }
}
